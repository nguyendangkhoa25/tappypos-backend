-- ============================================================
-- TENANT DATABASE — DEFAULT DATA: PAWN SHOP (TIỆM CẦM ĐỒ)
...
-- ── 1. Shop info ──────────────────────────────────────────────
INSERT INTO `shop_info` (`shop_name`) VALUES ('Tiệm Cầm Đồ');

-- ── 2. Product types (common pawn item types) ─────────────────
...
-- ── 3. Categories (pawn shop items) ──────────────────────────
INSERT INTO `category` (`id`, `name`, `parent_id`) VALUES
    (1,  'Điện tử',             NULL),
    (2,  'Trang sức',           NULL),
    (3,  'Xe máy / Xe đạp',    NULL),
    (4,  'Đồ gia dụng',        NULL),
    (5,  'Thời trang',          NULL),
    (6,  'Khác',                NULL),
    -- Điện tử sub-categories
    (7,  'Điện thoại',         1),
    (8,  'Laptop / Máy tính',  1),
    (9,  'Máy tính bảng',      1),
    (10, 'Đồng hồ thông minh', 1),
    (11, 'Máy ảnh',            1),
    (12, 'Thiết bị âm thanh',  1),
    -- Trang sức sub-categories
    (13, 'Vàng 24K',           2),
    (14, 'Vàng 18K / 14K',     2),
    (15, 'Bạc',                2),
    (16, 'Đồng hồ đeo tay',    2),
    -- Xe sub-categories
    (17, 'Xe máy',             3),
    (18, 'Xe đạp điện',        3)
ON DUPLICATE KEY UPDATE name = VALUES(name);
...
VALUES
    ...
    ('Bạc',       5000000,   1.25, '#9E9E9E', 'Chi tiêu từ 5 triệu VND',  2),
    ('Vàng',      20000000,  1.50, '#FFC107', 'Chi tiêu từ 20 triệu VND', 3),
    ('Kim cương', 100000000, 2.00, '#00BCD4', 'Chi tiêu từ 100 triệu VND',4);

-- ── 7. Default print template ────────────────────────────────
...
-- ── 8. Attribute groups & definitions (ELECTRONICS type) ─────
INSERT INTO `attribute_group` (`product_type_id`, `code`, `name`, `display_order`)
SELECT id, 'basic_info', 'Thông tin cơ bản', 1 FROM `product_type` WHERE code = 'ELECTRONICS'
ON DUPLICATE KEY UPDATE display_order = 1;

INSERT INTO `attribute_group` (`product_type_id`, `code`, `name`, `display_order`)
SELECT id, 'condition', 'Tình trạng', 2 FROM `product_type` WHERE code = 'ELECTRONICS'
...
SELECT pt.id, ag.id, 'brand', 'Hãng sản xuất', 'STRING', TRUE, TRUE, TRUE, 1
FROM `product_type` pt
JOIN `attribute_group` ag ON ag.product_type_id = pt.id AND ag.code = 'basic_info'
WHERE pt.code = 'ELECTRONICS'
ON DUPLICATE KEY UPDATE name = VALUES(name);

INSERT INTO `attribute_definition`
    (`product_type_id`, `attribute_group_id`, `code`, `name`, `data_type`,
     `required`, `searchable`, `filterable`, `display_order`)
SELECT pt.id, ag.id, 'model', 'Model / Phiên bản', 'STRING', TRUE, TRUE, TRUE, 2
FROM `product_type` pt
JOIN `attribute_group` ag ON ag.product_type_id = pt.id AND ag.code = 'basic_info'
WHERE pt.code = 'ELECTRONICS'
ON DUPLICATE KEY UPDATE name = VALUES(name);
...
INSERT INTO `attribute_definition`
    (`product_type_id`, `attribute_group_id`, `code`, `name`, `data_type`,
     `required`, `searchable`, `filterable`, `display_order`)
SELECT pt.id, ag.id, 'serial_number', 'Số serial / IMEI', 'STRING', FALSE, TRUE, FALSE, 3
FROM `product_type` pt
JOIN `attribute_group` ag ON ag.product_type_id = pt.id AND ag.code = 'basic_info'
WHERE pt.code = 'ELECTRONICS'
ON DUPLICATE KEY UPDATE name = VALUES(name);
...
INSERT INTO `attribute_definition`
    (`product_type_id`, `attribute_group_id`, `code`, `name`, `data_type`,
     `required`, `searchable`, `filterable`, `display_order`)
SELECT pt.id, ag.id, 'condition_grade', 'Tình trạng', 'STRING', TRUE, TRUE, TRUE, 1
FROM `product_type` pt
JOIN `attribute_group` ag ON ag.product_type_id = pt.id AND ag.code = 'condition'
WHERE pt.code = 'ELECTRONICS'
ON DUPLICATE KEY UPDATE name = VALUES(name);

-- ── 9. Banks (Vietnamese bank reference list) ────────────────
