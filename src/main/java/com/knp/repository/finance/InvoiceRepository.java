...
public interface InvoiceRepository extends JpaRepository<Invoice, Long> {
    ...
    @Query("SELECT COUNT(i) FROM Invoice i WHERE i.deleted = false AND i.createdAt >= :fromDate AND i.createdAt <= :toDate")
    long countInvoicesInRange(@Param("fromDate") LocalDateTime fromDate, @Param("toDate") LocalDateTime toDate);

    @Query("SELECT COALESCE(SUM(i.totalAmount), 0) FROM Invoice i WHERE i.deleted = false AND i.createdAt >= :fromDate AND i.createdAt <= :toDate")
    BigDecimal sumInvoicesInRange(@Param("fromDate") LocalDateTime fromDate, @Param("toDate") LocalDateTime toDate);
}
