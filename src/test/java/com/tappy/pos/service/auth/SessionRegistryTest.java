...
class SessionRegistryTest {
    ...
    void setUp() {
        ...
        lenient().when(activeSessionRepository.findByUsernameAndTenantId(any(), any())).thenReturn(Optional.empty());
    ...
    void remove_clearsSession() {
        registry.register("tenant1", "user1", sessionInfo);
        doNothing().when(activeSessionRepository).deleteByUsernameAndTenantId("user1", "tenant1");

        registry.remove("tenant1", "user1");

        assertThat(registry.getSession("tenant1", "user1")).isEmpty();
        verify(activeSessionRepository).deleteByUsernameAndTenantId("user1", "tenant1");
    }
    ...
    void register_updatesExistingDbRow() {
        ...
        when(activeSessionRepository.findByUsernameAndTenantId("user1", "tenant1")).thenReturn(Optional.of(existing));
